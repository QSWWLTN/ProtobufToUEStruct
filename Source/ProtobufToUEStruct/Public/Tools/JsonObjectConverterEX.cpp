...
FString Q_FJsonObjectConverterEX::StandardizeCase(const FString &StringIn)
{
...
TSharedPtr<FJsonValue> ConvertScalarUPropertyToJsonValue(FProperty* Property, const void* Value, int64 CheckFlags, int64 SkipFlags, const Q_FJsonObjectConverterEX::CustomExportCallback* ExportCb)
{
    ...
    else if (FArrayProperty *ArrayProperty = CastField<FArrayProperty>(Property))
    {
        ...
        {
            TSharedPtr<FJsonValue> Elem = Q_FJsonObjectConverterEX::UPropertyToJsonValue(ArrayProperty->Inner, Helper.GetRawPtr(i), CheckFlags & ( ~CPF_ParmFlags ), SkipFlags, ExportCb);
            if ( Elem.IsValid() )
            {
                // add to the array
                Out.Push(Elem);
    ...
    else if ( FSetProperty* SetProperty = CastField<FSetProperty>(Property) )
    {
        ...
        for ( int32 i=0, n=Helper.Num(); n; ++i )
        {
            ...
            {
                TSharedPtr<FJsonValue> Elem = Q_FJsonObjectConverterEX::UPropertyToJsonValue(SetProperty->ElementProp, Helper.GetElementPtr(i), CheckFlags & ( ~CPF_ParmFlags ), SkipFlags, ExportCb);
                if ( Elem.IsValid() )
                {
                    // add to the array
                    Out.Push(Elem);
    ...
    else if ( FMapProperty* MapProperty = CastField<FMapProperty>(Property) )
    {
        ...
        for ( int32 i=0, n = Helper.Num(); n; ++i )
        {
            if ( Helper.IsValidIndex(i) )
            {
                TSharedPtr<FJsonValue> KeyElement = Q_FJsonObjectConverterEX::UPropertyToJsonValue(MapProperty->KeyProp, Helper.GetKeyPtr(i), CheckFlags & ( ~CPF_ParmFlags ), SkipFlags, ExportCb);
                TSharedPtr<FJsonValue> ValueElement = Q_FJsonObjectConverterEX::UPropertyToJsonValue(MapProperty->ValueProp, Helper.GetValuePtr(i), CheckFlags & ( ~CPF_ParmFlags ), SkipFlags, ExportCb);
    ...
    else if (FStructProperty *StructProperty = CastField<FStructProperty>(Property))
    {
        ...
        TSharedRef<FJsonObject> Out = MakeShared<FJsonObject>();
        if (Q_FJsonObjectConverterEX::UStructToJsonObject(StructProperty->Struct, Value, Out, CheckFlags & (~CPF_ParmFlags), SkipFlags, ExportCb))
        {
            return MakeShared<FJsonValueObject>(Out);
        }
        // fall through to default
...
TSharedPtr<FJsonValue> Q_FJsonObjectConverterEX::ObjectJsonCallback(FProperty* Property, const void* Value)
{
    if (FObjectProperty* ObjectProperty = CastField<FObjectProperty>(Property))
    {
        if (!ObjectProperty->HasAnyFlags(RF_Transient)) // We are taking Transient to mean we don't want to serialize to Json either (could make a new flag if nessasary)
        {
            ...
            CustomCB.BindStatic(Q_FJsonObjectConverterEX::ObjectJsonCallback);

            void** PtrToValuePtr = (void**)Value;

            if (Q_FJsonObjectConverterEX::UStructToJsonObject(ObjectProperty->PropertyClass, (*PtrToValuePtr), Out, 0, 0, &CustomCB))
            {
                return MakeShared<FJsonValueObject>(Out);
            }
        }
...
TSharedPtr<FJsonValue> Q_FJsonObjectConverterEX::UPropertyToJsonValue(FProperty* Property, const void* Value, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb)
{
...
bool Q_FJsonObjectConverterEX::UStructToJsonObject(const UStruct* StructDefinition, const void* Struct, TSharedRef<FJsonObject> OutJsonObject, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb)
{
    return UStructToJsonAttributes(StructDefinition, Struct, OutJsonObject->Values, CheckFlags, SkipFlags, ExportCb);
}

bool Q_FJsonObjectConverterEX::UStructToJsonAttributes(const UStruct* StructDefinition, const void* Struct, TMap< FString, TSharedPtr<FJsonValue> >& OutJsonAttributes, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb)
{
    if (SkipFlags == 0)
    {
...
bool Q_FJsonObjectConverterEX::UStructToJsonObjectString(const UStruct* StructDefinition, const void* Struct, FString& OutJsonString, int64 CheckFlags, int64 SkipFlags, int32 Indent, const CustomExportCallback* ExportCb, bool bPrettyPrint)
{
...
//static
bool Q_FJsonObjectConverterEX::GetTextFromObject(const TSharedRef<FJsonObject>& Obj, FText& TextOut)
{
    ...
    bool ConvertScalarJsonValueToUPropertyWithContainer(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* OutValue, const UStruct* ContainerStruct, void* Container, int64 CheckFlags, int64 SkipFlags)
    {
        ...
        else if (FTextProperty* TextProperty = CastField<FTextProperty>(Property))
        {
            ...
            else if (JsonValue->Type == EJson::Object)
            {
                ...
                FText Text;
                if (!Q_FJsonObjectConverterEX::GetTextFromObject(Obj.ToSharedRef(), Text))
                {
...
bool Q_FJsonObjectConverterEX::JsonValueToUProperty(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* OutValue, int64 CheckFlags, int64 SkipFlags)
{
    return JsonValueToUPropertyWithContainer(JsonValue, Property, OutValue, nullptr, nullptr, CheckFlags, SkipFlags);
}

bool Q_FJsonObjectConverterEX::JsonObjectToUStruct(const TSharedRef<FJsonObject>& JsonObject, const UStruct* StructDefinition, void* OutStruct, int64 CheckFlags, int64 SkipFlags)
{
    return JsonAttributesToUStruct(JsonObject->Values, StructDefinition, OutStruct, CheckFlags, SkipFlags);
}

bool Q_FJsonObjectConverterEX::JsonAttributesToUStruct(const TMap< FString, TSharedPtr<FJsonValue> >& JsonAttributes, const UStruct* StructDefinition, void* OutStruct, int64 CheckFlags, int64 SkipFlags)
{
...
FFormatNamedArguments Q_FJsonObjectConverterEX::ParseTextArgumentsFromJson(const TSharedPtr<const FJsonObject>& JsonObject)
{
    ...
    if (JsonObject.IsValid())
    {
        for (const auto& It : JsonObject->Values)
        {
            ...
            case EJson::Object:
            {
                // localized string
                FText TextOut;
                if (Q_FJsonObjectConverterEX::GetTextFromObject(It.Value->AsObject().ToSharedRef(), TextOut))
                {
